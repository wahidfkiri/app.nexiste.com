...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .addr-lines { font-size: 8.8pt; line-height: 1.55; }
      ...
      <table class="table">
        <tr>
          <td style="width:60%;vertical-align:top;">
            ...
            @endif
            <div style="font-size:16pt;font-weight:bold;color:{{ $palette['primary'] }};">{{ $invoice->tenant->name ?? config('app.name') }}</div>
          ...
          <td style="width:40%;vertical-align:top;text-align:right;">
            ...
            @if($invoice->reference)<div class="muted" style="font-size:8.6pt;margin-top:4px;">Référence : {{ $invoice->reference }}</div>@endif
            <span class="pill pill-{{ $status }}">{{ $invoice->status_label ?? $status }}</span>
          </td>
        </tr>
      </table>
      ...
      <table class="addr">
        <tr>
          <td>
            ...
            <div class="addr-lines">{{ $invoice->tenant->address ?? '' }}<br>{{ $invoice->tenant->email ?? '' }}</div>
          </td>
          <td>
